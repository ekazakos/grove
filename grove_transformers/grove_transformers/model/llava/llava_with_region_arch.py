...
class LlavaMetaModel:
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            # self.llm_projector = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        vision_tower = build_vision_tower(model_args, delay_load=True)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        image_features=None, image_forward_outs=None, images_dtype=None, token_embeddings=None,
    ):
        ...
        if images is not None and ((isinstance(images, list)) or (hasattr(images, 'ndim') and images.ndim == 6)):
            ...
            if self.config.with_region:
                ...
                mlvl_reg_features = [item[:, 1:].to(images_dtype) for item in mlvl_reg_features]
        ...
        for batch_idx, (cur_input_ids, reg_feat) in enumerate(zip(input_ids, mlvl_reg_features)): # Adjusted the loop to include reg_feat
            ...
            if (cur_input_ids == IMAGE_TOKEN_INDEX).sum() == 0:
                ...
                if token_embeddings is not None:
                    cur_input_embeds = token_embeddings[cur_input_ids]
                else:
                ...
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    ...
                    if token_embeddings is not None:
                        cur_new_input_embeds.append(
                            token_embeddings[cur_input_ids[: image_token_start - 1]]
                        )
                        cur_new_input_embeds.append(
                            token_embeddings[cur_input_ids[image_token_start - 1 : image_token_start]]
                        )
                    else:
                    ...
                    if token_embeddings is not None:
                        cur_new_input_embeds.append(
                            token_embeddings[cur_input_ids[image_token_start + 1 : image_token_start + 2]]
                        )
                    else:
                        cur_new_input_embeds.append(
                            self.get_model().embed_tokens(
                    ...
                    if token_embeddings is not None:
                        cur_new_input_embeds.append(
                            token_embeddings[cur_input_ids[: image_token_start]]
                        )
                    else:
                        cur_new_input_embeds.append(
            ...
            if cur_input_ids.numel() > 0:
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    ...
                    if token_embeddings is not None:
                        cur_new_input_embeds.append(
                            token_embeddings[cur_input_ids]
                        )
                    else:
                        cur_new_input_embeds.append(
