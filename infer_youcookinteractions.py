...
def parse_args():
    parser = argparse.ArgumentParser(description="GROVE Inference - YouCookInteractions")
    ...
    parser.add_argument("--video_dir", default="/home/YouCookIIVideos/val", type=str)
    parser.add_argument("--video_info", default="/home/GroundingYouTube/data/Youcook2/annotation/YouCook2-Interactions/full_annotations.pkl", type=str)
    parser.add_argument("--output_path", type=Path, default=Path("/home/grove_inference_output/result_youcookinteractions.pkl"))
...
class YouCookInteractionsInferenceDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        video_dir,
        ann_path,
    ...
    def __getitem__(self, idx):
        ...
        pts_list = video['pts']
        target_secs = video['secs_relative']
        video_fps = video["fps"]
        w = video["width"]
        h = video["height"]
        unique_clip_id = f"{video["video_id"]}_{video["segment_youcook_idx"]}"
...
if __name__ == "__main__":
    ...
    # Create DDP Dataset
    dataset = YouCookInteractionsInferenceDataset(args.video_dir, args.video_info)
