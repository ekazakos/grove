...
job_id2=$(sbatch --parsable --dependency=afterok:$job_id1 infer_eval_scripts/infer_youcookinteractions.sh $GROVE_WEIGHTS $TOKEN_EMBEDDINGS_PATH $OUTPUT_PATH $GT_FILE_PATH $VIDEO_DIR_PATH)

# Submit job for evaluation
sbatch --dependency=afterok:$job_id2 infer_eval_scripts/eval_youcookinteractions.sh $OUTPUT_PATH $GT_FILE_PATH $TOKEN_EMBEDDINGS_PATH
